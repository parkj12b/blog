---
title: "소개"
date: 2026-02-11
draft: false
showComments: false
showPagination: false
---

안녕하세요. 리눅스 커널을 공부하고 있는 개발지망생 박민서 입니다. 궁금한 점이 있으시다면 위에 연락처로 문의주세요. 감사합니다! 

## 보유 기술 (Skills)
* **Languages:** C, C++, Python, x86 Assembly (NASM), Bash
* **Kernel & OS:** Linux Kernel Development, Memory Management (mm), LKM, Xv6, Syzkaller, strace, GDB
* **Virtualization & DevOps:** Docker, Docker Compose, QEMU, VirtualBox
* **System Programming:** Socket Programming, Multi-threading (pthreads, OpenMP), Custom Malloc, IPC
* **Architecture:** x86, Computer Architecture (Pipelining, Cache), SIMD (AVX), AVR C
* **Tools:** Git, Make, GCC, Flex, Bison, Valgrind, CAN

## 주요 이력 (Timeline)
* **2019년 9월 - 2020년 12월**: 워털루 대학교 (University of Waterloo) - 메카트로닉스 공학 | 캐나다 온타리오주 워털루 (중퇴)
* **2021년 6월 - 2021년 12월**: 병역 의무 대기 기간
* **2021년 12월 - 2022년 8월**: 대한민국 육군 (의무 복무) | 대한민국 대구
* **2023년 8월 - 2025년 7월**: 42 서울 (프로그래밍 학교) | 대한민국 서울 
  * 에꼴 42(Ecole 42)의 한국 캠퍼스
* **2025년 7월 - 2025년 9월**: 42 볼프스부르크 (42 Wolfsburg, SEA:ME) | 독일 볼프스부르크
* **2025년 10월 - 현재**: 독립 연구 및 학습 | 대한민국 전주

## 참여 프로그램 (Programs)
* **Linux Kernel Mentorship Program (파트타임)** | 원격
  * **2026년 3월 - 2026년 8월**
  * Linux Foundation 멘토이자 Linux 커널 메인테이너인 Shuah Khan과 함께하는 Linux 커널 멘토십 프로그램.
  * 커널 기여에 대한 지도와 질의응답을 위해 매주 오피스 아워 진행.
  * linux-mm 메모리 메니저 공부와 syzbot 을 통한 bug fixing 에 몰두중.

## 개인 프로젝트 (Personal Projects)
* **B** - B 언어 컴파일러를 i386 (x86) 어셈블리로 재구현. Flex 및 Bison LALR(1)을 사용하여 싱글 패스 컴파일러(Single Pass Compiler) 구현.
* **Linux From Scratch** - 소스 코드부터 완전히 사용자 정의된 리눅스 배포판 구축. `dmesg`, `earlyprintk` 및 GRUB/EFI 로그를 사용하여 부팅 실패 디버깅. VirtualBox 내부 및 베어 메탈 환경에서 커널 동작 검증.
* **Libasm** - x86 어셈블리로 `strlen` 및 정렬 알고리즘과 같은 간단한 함수 코딩. SIMD (Intel AVX) 프로그래밍 실험 (Intel 문법의 NASM 사용).
* **Malloc** - 동적 메모리 할당의 사용자 수준 관리(user-level management) 구현 및 탐구.
* **Process and Memory** - 리눅스 커널에 시스템 콜 구현 및 추가.
* **Ft_fracol** - 다양한 프랙탈 구조를 연구하고 Minilibx 라이브러리를 사용하여 시각화. OpenMP를 활용하여 직렬 루프를 병렬화하고 효율적인 멀티스레드 실행 활성화.
* **Little-penguine-1** - Eudyptula Challenge 기반의 간단한 리눅스 디바이스 드라이버 작성.
* **Drivers_and_interrupts** - 리눅스 커널용 LKM(Loadable Kernel Module) 형태의 커널 수준 키로거 작성.
* **Webserver** - C++를 사용하여 소켓 프로그래밍 기반의 웹 서버 구현. Nginx와 유사한 설정 파일을 처리하기 위해 재귀 하강 파싱(Recursive Descent Parsing) 기법을 적용한 파서 제작.

## 개인 공부 (Independent Study)
* **운영 체제 공학 (Operating System Engineering)** - MIT OCW (6.1810)를 통해 Xv6 커널 개발 랩을 수행하여 OS 아키처 시뮬레이션 및 디버깅. 시스템 콜, 페이지 테이블, 트랩, Copy-on-write, 네트워크 드라이버, Readers-writer lock, 파일 시스템, mmap 등 다양한 OS 개념 탐구 및 구현.
* **컴퓨터 아키텍처 (Computer Architecture)** - Coursera에서 제공하는 프린스턴 대학교(Princeton)의 ELE 475 과정 수강. 파이프라이닝(Pipelining), 슈퍼스칼라(Superscalar), VLIW, 캐시(Cache), 분기 예측(Branch Prediction) 등 고급 주제 학습.
* **AVR C를 활용한 Arduino (Arduino with AVR C)** - Arduino Uno (atmega328p)에서 AVR C를 사용한 베어 메탈 프로그래밍 탐구.
* **운영 체제 구조 (Operating System Organization, CSE422)** - 리눅스 커널 실습을 포함하는 워싱턴 대학교(Washington University) 운영 체제 강의 수강.
* **리눅스 메모리 관리자 (The Linux Memory Manager)** - 버전 6.x 기반의 리눅스 커널 메모리 관리자 상세 학습.
